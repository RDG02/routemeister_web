...
def test_planning_all_patients():
    print("=== STARTING PLANNING WITH ALL PATIENTS TEST ===")
    
    # Step 1: Check data
    print("\n1. Checking data...")
    patients = Patient.objects.all()  # Use ALL patients, not just 'nieuw'
    vehicles = Vehicle.objects.filter(status='beschikbaar')
    
    print(f"   - All patients: {patients.count()}")
    ...
    for timeslot_name, patients_list in patients_by_halen.items():
        ...
        for p in patients_list:
            print(f"       - {p.naam} (pickup: {p.ophaal_tijd.time()}, status: {p.status})")
