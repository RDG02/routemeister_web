...
class TimeSlot(models.Model):
    ...
    """
    naam = models.CharField(max_length=100, help_text="Naam van het tijdblok (bijv. '08:00 Uhr')")
    
    # Type tijdblok
    TIJDBLOK_TYPE_CHOICES = [
        ('halen', 'Halen'),
        ('brengen', 'Brengen'),
    ]
    tijdblok_type = models.CharField(max_length=10, choices=TIJDBLOK_TYPE_CHOICES, default='halen', help_text="Type tijdblok")
    
    # Tijd van aankomst bij reha center (voor halen) of eind tijd voor brengen (voor brengen)
    aankomst_tijd = models.TimeField(default=time(8, 0), help_text="Tijd van aankomst reha center (halen) of eind tijd voor brengen (brengen)")
    ...
    def __str__(self):
        return f"{self.naam} ({self.get_tijdblok_type_display()}) - {self.aankomst_tijd}"
    
    def get_duration_minutes(self):
        """Bereken de duur van het tijdblok in minuten"""
        from datetime import datetime, timedelta
        # Voor de nieuwe structuur is dit niet meer relevant, maar behouden voor compatibiliteit
        return 90  # Standaard 90 minuten voor een tijdblok
    
    class Meta:
        verbose_name = "Tijdblok"
        verbose_name_plural = "Tijdblokken"
        ordering = ['tijdblok_type', 'aankomst_tijd']
...
class PlanningConstraint(models.Model):
    """
    Model voor het configureren van planning constraints in de Django admin
    """
    name = models.CharField(max_length=100, help_text="Naam van de constraint")
    description = models.TextField(help_text="Beschrijving van wat de constraint doet")
    
    CONSTRAINT_TYPES = [
        ('vehicle_capacity', 'Voertuig Capaciteit'),
        ('vehicle_special_capacity', 'Voertuig Speciale Capaciteit'),
        ('vehicle_drive_time', 'Voertuig Rijtijd'),
        ('distance_cost', 'Afstand Kosten'),
        ('preferred_vehicle', 'Voorkeurs Voertuig'),
        ('drv_count', 'DRV Aantal'),
    ]
    
    constraint_type = models.CharField(
        max_length=50, 
        choices=CONSTRAINT_TYPES,
        help_text="Type constraint"
    )
    ...
    weight = models.CharField(
        max_length=10,
        choices=CONSTRAINT_WEIGHTS,
        default='HARD',
        help_text="Hard = mag niet overtreden, Soft = optimalisatie"
    )
    
    penalty = models.IntegerField(
        default=1000,
        help_text="Penalty waarde voor constraint violation"
    )
    
    is_active = models.BooleanField(
        default=True,
        help_text="Of deze constraint actief is"
    )
    
    parameters = models.JSONField(
        default=dict,
        blank=True,
        help_text="Extra parameters voor de constraint (JSON)"
    )
    ...
    def __str__(self):
        return f"{self.get_constraint_type_display()} - {self.name}"
    
    def get_penalty_display(self):
        """Toon penalty in leesbaar formaat"""
        if self.penalty >= 1000000:
            return f"{self.penalty // 1000000}M"
        elif self.penalty >= 1000:
