...
def test_planning_csv_only():
    print("=== STARTING PLANNING WITH CSV PATIENTS ONLY ===")
    
    # Step 1: Check data - ONLY CSV patients with status 'nieuw'
    print("\n1. Checking CSV data...")
    csv_patients = Patient.objects.filter(status='nieuw')  # Only patients from uploaded CSV
    vehicles = Vehicle.objects.filter(status='beschikbaar')
    
    print(f"   - CSV patients (status 'nieuw'): {csv_patients.count()}")
    print(f"   - Vehicles: {vehicles.count()}")
    
    # Show which patients we're using
    print("\n   CSV patients being used:")
    ...
    patients_with_timeslots = csv_patients.filter(
        models.Q(halen_tijdblok__isnull=False) | 
        models.Q(bringen_tijdblok__isnull=False)
    ).distinct()
    print(f"   - CSV patients with time slots: {patients_with_timeslots.count()}")
    
    if patients_with_timeslots.count() == 0:
        print("   - WARNING: No CSV patients have time slots assigned!")
        return
    ...
    # Step 6: Group CSV patients by halen time slot
    print("\n6. Grouping CSV patients by halen time slot...")
    patients_by_halen = {}
    ...
    # Step 7: Send CSV patients per time slot
    print("\n7. Sending CSV patients per time slot...")
    ...
    print(f"\n   - Total CSV patients added: {total_patients_added}")
